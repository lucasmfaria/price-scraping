...
while True:
    ...
    if event == "-PRECOS FILENAME-":
        ...
        try:
            df_precos_parcial = carrega_lista_precos(Path(precos_input_path), config)
        except:
            print("Não foi possível ler o arquivo .csv")
        else:
            ...
            for idx, row in df_cards.iterrows():  # busca cada 'nome' em df_cards
                if (row['nome'], row['num_colecao']) not in cards_ja_buscados:
                    ...
                    if colecao:
                        ...
                        constroi_resultados(precos_website_1, df_precos_parcial, df_cards, config, Path(csv_input_path))
                    else:
